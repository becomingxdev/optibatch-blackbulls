...
const Chatbot = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setMessages(prev => [
        ...prev, 
        { role: 'assistant', text: data.response || "Communication offline." }
      ]);
    } catch (err) {
      setMessages(prev => [
        ...prev,
        { role: 'assistant', text: 'Error connecting to the AI Subsystem.' }
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <>
      ...
      {!isOpen && (
        ...
        >
          <div className="absolute inset-0 bg-cyan-400/20 rounded-full blur-md opacity-0 group-hover:opacity-100 transition duration-300"></div>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-8 right-8 w-full max-w-sm sm:max-w-md h-[500px] max-h-[80vh] bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden z-50 animate-in slide-in-from-bottom-5 fade-in duration-300">
          
          {/* Header */}
          <div className="px-5 py-4 border-b border-white/10 bg-slate-900/50 flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="relative group flex items-center justify-center p-2 rounded-lg bg-cyan-900/50 border border-cyan-500/30">
                <Cpu size={18} className="text-cyan-400 animate-pulse" />
              </div>
              <div>
                <h3 className="font-black text-white tracking-tight flex items-center gap-2">OPTI<span className="text-cyan-400">AI</span></h3>
                ...
                <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg border ${
                    msg.role === 'user' 
                      ...
                      : 'bg-cyan-900/30 border-cyan-500/20 text-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.1)]'
                  }`}>
                    {msg.role === 'user' ? <User size={16} /> : <Bot size={18} />}
                  </div>

                  {/* Bubble */}
                  <div className={`p-3 rounded-2xl text-[13px] leading-relaxed backdrop-blur-md ${
                    msg.role === 'user'
                      ? 'bg-slate-800/80 text-white rounded-tr-sm border border-slate-700'
                      : 'bg-cyan-950/20 text-slate-200 rounded-tl-sm border border-cyan-500/10'
                  }`}>
                    {msg.text}
                  </div>
                </div>
          ...
          <div className="p-4 border-t border-white/10 bg-slate-900/80 backdrop-blur-md">
            <form onSubmit={handleSendMessage} className="relative flex items-center">
              ...
              <button 
                ...
                className="absolute right-2 p-2 rounded-lg bg-cyan-500/10 text-cyan-400 hover:bg-cyan-400 hover:text-slate-900 disabled:opacity-50 disabled:hover:bg-cyan-500/10 disabled:hover:text-cyan-400 transition-colors"
              >
